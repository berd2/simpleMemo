...
*   **오른쪽 패널 (편집)**:
    ...
    *   **편집 영역**: 메모의 전체 내용을 보고 편집하는 공간입니다. 첫 번째 줄은 자동으로 굵고 큰 글씨의 제목 스타일이 적용됩니다. 또한 마크다운(Markdown) 문법을 지원하여 가독성 높은 메모 작성이 가능합니다.
...
*   **자동 저장**:
    *   메모를 편집하던 중 왼쪽 목록에서 다른 메모를 선택하거나, 메모장 창을 닫을 때, 혹은 **타이핑을 멈추고 2초가 지나면** 현재 편집 중인 내용이 **자동으로 저장**되므로 데이터가 유실될 걱정이 없습니다.

*   **마크다운 및 체크리스트 지원**:
    *   마크다운 하이라이팅: `# 제목`, `**굵게**`, `*기울임*`, `~~취소선~~`, `` `코드` `` 등의 마크다운 문법을 작성하면 색상이 하이라이팅 됩니다.
    *   클릭 가능한 체크리스트: `- [ ] 할 일` 과 같이 체크리스트를 작성한 뒤 편집창에서 체크박스 부분을 클릭하면 `- [x]` 형태로 토글됩니다.
